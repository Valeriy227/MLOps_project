...
for col in df.columns:
    print('percentage of NAs in {} is {:.1f}%'.format(
        col, 100. * df[col].isna().sum() / len(df)))
...
df['duration'] = (df.tpep_dropoff_datetime -
                  df.tpep_pickup_datetime) / datetime.timedelta(minutes=1)
print('Dropping tpep_pickup_datetime and tpep_dropoff_datetime')
pickup_day = df.tpep_pickup_datetime.dt.day
df.drop(columns=['tpep_pickup_datetime',
        'tpep_dropoff_datetime'], inplace=True)
...
df_train = result[pickup_day < 28]
df_test = result[pickup_day >= 28]
print("Divide data to train and test by 28.01")
print(f"Train size: {df_train.shape[0]}")
print(f"Test size: {df_test.shape[0]}")

df_train.to_parquet("data_train.parquet")
df_test.to_parquet("data_test.parquet")
